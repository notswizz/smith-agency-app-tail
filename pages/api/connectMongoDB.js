...
export default async function handler(req, res) {
  try {
    // Log the incoming request method and timestamp
    const timestamp = new Date().toLocaleString();
    console.log(`Received a ${req.method} request to the MongoDB handler at ${timestamp}`);
    ...
    // Log the error and timestamp
    const timestamp = new Date().toLocaleString();
    console.error(`[${timestamp}] MongoDB connection or operation error:`, error);

    // Check if the error is related to the MongoDB client
    if (!client.isConnected()) {
      console.error(`[${timestamp}] MongoDB client is not connected`);
    }
